...
def masterLoop_2():
    ...
    r = p['sy']['pop']()
    logg('processing text -- exiting -- ') 
    if (r == p['OK']):
        logg('push text exiting ')
        datPush("exiting")
        logg('after exiting ')
        datPush(p['OK'])
    else:
...
def masterLoop_9():
    ...
    r = p['sy']['pop']()
    logg('processing text -- dumpDAT -- ') 
    if (r == p['OK']):
        logg('push text dumpDAT ')
        datPush("dumpDAT")
        logg('after dumpDAT ')
        datPush(p['OK'])
    else:
...
def masterLoop (x):
    ...
    while masterLoopCtl != 0:
        ...
        elif (masterLoopCtl == 7):
            logg('call masterLoop_7')
            masterLoop_7()
            logg('after call masterLoop_7')
            # test and adjust for new spoke
            masterLoopCtl = chk(masterLoopCtl)

        elif (masterLoopCtl == 8):
            logg('call masterLoop_8')
            masterLoop_8()
            logg('after call masterLoop_8')
            # test and adjust for new spoke
            masterLoopCtl = chk(masterLoopCtl)

        elif (masterLoopCtl == 9):
            logg('call masterLoop_9')
            masterLoop_9()
            logg('after call masterLoop_9')
            # test and adjust for new spoke
            masterLoopCtl = chk(masterLoopCtl)
